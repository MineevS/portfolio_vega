<?php
    class AUTH_PORTFOLIO {
        ...
        function save_data_profile(){
            ...
            foreach($query->iterateKeyValue() as $cn => $type){
                ...
                if($status_key_in_post){
                    ...
                    switch($cn){
                        case 'icon':
                            $this->save_img('./../../frontend/icons/avatars_profiles',  /* Указать, в какую директорию загружать */
                                \TBN::PROFILES->value, $_SESSION['id'], $cn);
                    ...
                    if($cn == 'id'    || $cn == 'login' || $cn == 'roles' 
                    || $cn == 'icon'  || $cn == 'hash'  || $cn == 'telephone' 
                    || $cn == 'email' || $cn == 'status' ) continue;
        ...
        function change_data($tab_name, $type_changed, $folder_for_load_avatar){
            ...
            foreach($query->iterateKeyValue() as $cn => $type){
                ...
                if($status_key_in_post){
                    ...
                    switch($cn){
                        case 'icon':
                            $this->save_img('./../../frontend/icons/' . $folder_for_load_avatar,  /* Указать, в какую директорию загружать */
                                $tab_name, $_POST['id'], $cn); // avatars_profiles // $_SESSION['id']
                            break;
                        default:
                            break;
                        }
        ...
        function create_project(){
            ...
            if(isset($_POST['icon'])){ array_push($list_properties, 'icon'); $_SESSION['icon-project'] = $_POST['icon']; }
        ...
        function getCountLikeForProjectId($pid){ // Для определения общего количества лайков на конкретный проект от всех пользователей, которые поставили соответствующий like.
            global $conn;
        ...
        function save_img($dir, $tab, $id, $col_name){
            global $conn;
